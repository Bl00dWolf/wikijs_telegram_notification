...
## Обнуление кеша 
Для обнуления кэша статей удалите файл
`.wiki_cache.json`
Его удаление заставит перечитать все статьи снова и при этом, первом перечитывании, о них не будет уведомлений в ТГ, чтобы избежать спама (т.к. все статьи разом иначе бы каждое в своем сообщении бы отправились ботом в ТГ)
